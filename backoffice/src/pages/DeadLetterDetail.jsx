    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400 font-medium">Loading dead letter...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-4 flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-900 font-medium">Dead Letter</span>
      </div>

      {/* Page header */}
      <div className="mb-6 animate-fade-in-up">
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Dead Letter</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6 animate-fade-in-up" style={{ animationDelay: '60ms' }}>
        <h2 className="text-lg font-semibold text-slate-900 mb-4">Dead Letter Metadata</h2>
        <dl className="grid grid-cols-2 gap-x-8 gap-y-4">
          <div>
            <dt className="text-sm font-medium text-slate-500">Dead Letter ID</dt>
            <dd className="text-base font-mono text-slate-900 mt-1">{deadLetter.id}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-slate-500">Original Message ID</dt>
          ...
          <div>
            <dt className="text-sm font-medium text-slate-500">Failed At</dt>
          ...
          {deadLetter.resolved && (
            <>
              <div>
                <dt className="text-sm font-medium text-slate-500">Resolved At</dt>
                <dd className="text-base text-slate-900 mt-1">
                  {deadLetter.resolvedAt ? new Date(deadLetter.resolvedAt).toLocaleString() : '-'}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-slate-500">Resolved By</dt>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-rose-200 p-6 mb-6 animate-fade-in-up" style={{ animationDelay: '120ms' }}>
        <h2 className="text-lg font-semibold text-rose-900 mb-4">Error Reason</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-fade-in-up" style={{ animationDelay: '180ms' }}>
        <h2 className="text-lg font-semibold text-slate-900 mb-4">Raw Payload</h2>
