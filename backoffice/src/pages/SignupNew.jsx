        ...
        <div className="flex items-center">
          {STEPS.map((s, i) => (
            <div key={s.key} className="flex items-center flex-1 last:flex-none">
              ...
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-4 rounded-full transition-colors duration-300 ${i < step ? 'bg-gradient-to-r from-emerald-400 to-teal-400' : 'bg-slate-100'}`} />
              )}
            </div>
          ))}
        </div>
...
function SummaryRow({ label, value, highlight }) {
  return (
    <div className="flex px-4 py-3">
      ...
      <span className={`text-sm ${highlight ? 'text-amber-600 italic font-medium' : 'text-slate-700'}`}>{value}</span>
    </div>
  );
}
