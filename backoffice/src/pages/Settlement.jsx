...
function StatusBadge({ status, label, styles }) {
  const cfg = styles[status] || Object.values(styles)[0];
  return (
...
function RunsTab({ t }) {
  ...
  const [gridAreaCode, setGridAreaCode] = useState('');
  const [fromDate, setFromDate] = useState('');
  const [toDate, setToDate] = useState('');

  const fetchPage = useCallback((p) => {
    setError(null);
    api.getSettlementRuns({
      status: status || undefined,
      meteringPointId: meteringPointId || undefined,
      gridAreaCode: gridAreaCode || undefined,
  ...
  }, [status, meteringPointId, gridAreaCode, fromDate, toDate]);

  useEffect(() => { setPage(1); }, [status, meteringPointId, gridAreaCode, fromDate, toDate]);
  ...
  if (loading && runs.length === 0) {
    return <div className="px-4 py-12 text-center text-slate-500">{t('common.loading')}</div>;
  }
    ...
    <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6 animate-fade-in-up" style={{ animationDelay: '40ms' }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          >
            <option value="">{t('settlement.allStatuses')}</option>
          ...
          <input
            type="text"
            placeholder={t('settlement.filterMeteringPoint')}
          ...
          <input
            type="text"
            placeholder={t('settlement.filterGridArea')}
            value={gridAreaCode}
            onChange={(e) => setGridAreaCode(e.target.value)}
            className="px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400"
          />
          <input
            type="date"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 animate-fade-in-up" style={{ animationDelay: '60ms' }}>
        <div className="bg-gradient-to-br from-white to-slate-50 rounded-xl p-5 shadow-sm border border-slate-100">
          <div className="text-sm font-medium text-slate-500 mb-1">{t('settlement.totalRuns')}</div>
        ...
        <div className="bg-gradient-to-br from-white to-teal-50/30 rounded-xl p-5 shadow-sm border border-teal-100/50">
          <div className="text-sm font-medium text-teal-600 mb-1">{t('settlement.runsOnPage')}</div>
          <div className="text-3xl font-bold text-teal-700">{runs.length}</div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colStatus')}</th>
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colMeteringPoint')}</th>
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colPeriod')}</th>
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colGridArea')}</th>
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colVersion')}</th>
                <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-slate-600 uppercase tracking-wider">{t('settlement.colExecutedAt')}</th>
              </tr>
            </thead>
                ...
                runs.map((run) => (
                  <tr key={run.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <StatusBadge status={run.status} label={t('status.' + run.status)} styles={runStatusStyles} />
                    </td>
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap text-sm text-slate-700">
                      <Link to={`/billing/runs/${run.id}`} className="text-teal-600 hover:text-teal-700">
                        {run.periodStart} — {run.periodEnd}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5 whitespace-nowrap text-sm text-slate-500">{run.gridAreaCode || '-'}</td>
                    <td className="px-4 py-2.5 whitespace-nowrap text-sm text-slate-500">v{run.version}</td>
        ...
        {totalPages > 1 && (
          <div className="px-5 py-3.5 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div className="text-sm text-slate-600">
              {t('common.showingRange', { from: (page - 1) * PAGE_SIZE + 1, to: Math.min(page * PAGE_SIZE, totalCount), total: totalCount })} {t('settlement.showingRuns')}
            </div>
...
export default function Settlement() {
  ...
  const tabs = [
    { key: 'runs', label: t('settlement.tabRuns') },
    { key: 'corrections', label: t('settlement.tabCorrections') },
  ];
  ...
  return (
    <div className="p-4 sm:p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6 animate-fade-in-up">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">{t('settlement.title')}</h1>
        <p className="text-base text-slate-500 mt-1">{t('settlement.subtitle')}</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-slate-100 rounded-xl p-1 w-fit animate-fade-in-up" style={{ animationDelay: '30ms' }}>
        {tabs.map(({ key, label }) => (
          <button
            key={key}
            onClick={() => setTab(key)}
      ...
      {/* Tab content */}
      {tab === 'runs' && <RunsTab t={t} />}
      {tab === 'corrections' && <CorrectionsTab t={t} />}
    </div>
  );
}
